...
if not GITHUB_URL_PAT.match(url):
    raise ValueError("illegal Github url: '{url}'")
...
if not LIB_NAME_PAT.match(package):
    raise ValueError("illegal package name: '{package}'")
...
if not EMAIL_PAT.match(email):
    raise ValueError("illegal email: '{email}'")
...
if not PYTHON_VERSION_PAT.match(version):
    raise ValueError("illegal Python version: '{version}'")
